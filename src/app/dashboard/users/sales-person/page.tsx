          ...
          <table className={styles.userTable}>
            ...
            <tbody>
              {salesPersons.map((person, index) => (
                <tr key={index}>
                  <td style={{ fontWeight: 600, color: "#94a3b8" }}>{person.id}</td>
                  <td>
                    <div className={styles.userDetails}>
                      ...
                      <span className={styles.userNameTable} style={{ color: "#1e293b", fontWeight: 600 }}>{person.name}</span>
                    </div>
                  </td>
                  <td style={{ fontWeight: 500, color: "#1e293b" }}>{person.mobile}</td>
                  <td style={{ color: "#64748b" }}>{person.email}</td>
                  <td style={{ fontWeight: 500, color: "#1e293b" }}>{person.company}</td>
                  <td style={{ fontWeight: 600, color: "#1e293b" }}>{person.activeLeads}</td>
                  <td style={{ fontWeight: 600, color: "#1e293b" }}>{person.customers}</td>
                  <td style={{ fontWeight: 600, color: "#1e293b" }}>{person.closedLeads}</td>
                  <td>
                    <div className={styles.statusCell}>
                      <span className={person.status === "Active" ? styles.statusDotActive : styles.statusDotInactive}></span>
                      {person.status}
                    </div>
                  </td>
                  <td style={{ overflow: "visible" }}>
                    <div onClick={(e) => {
                      e.stopPropagation();
                      setOpenActionId(openActionId === person.id + index ? null : person.id + index);
                    }}>
                      <MoreVertical size={18} color="#94a3b8" cursor="pointer" />
                    </div>
                    
                    {openActionId === person.id + index && (
